...
const geomap = protoDescriptor.geomap as any;

const geoClient = new geomap.GeoService(
  'localhost:50052',
...
function getGeoUserData(userId: string): Promise<any> {
  return new Promise((resolve, reject) => {
    geoClient.GetGeoUserData({ user_id: userId }, (error: any, response: any) => {
      if (error) {
